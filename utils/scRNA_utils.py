...
def paird_ttest(adata, condition_key = None, sample_id_col = None, patient_id_col = None, pval_cutoff = 0.05, log2fc_cutoff = 1):
    ...
    # Clean and prepare p-values for q-value calculation
    res_df['pval'] = res_df['pval'].replace([np.inf, -np.inf], np.nan)  # Replace infinite values with NaN
    res_df = res_df.dropna(subset=['pval'])  # Drop rows with NaN in 'pval'
    res_df['pval'] = res_df['pval'].clip(0, 1)  # Ensure p-values are within [0, 1]
...
def add_missing_samples(adata, missing_sample_ids):
    """
    Add missing samples with zero expression to an AnnData object.
    ...
    """
    if len(missing_sample_ids) > 0:
        n_genes = adata.shape[1]
        zeros_data = np.zeros((len(missing_sample_ids), n_genes))
        
        # Extract timepoint from sample_id
        timepoints = [sample_id.split('_')[-1] for sample_id in missing_sample_ids]
        timepoints = [timepoint.lower() for timepoint in timepoints]
        missing_obs = pd.DataFrame({
            'sample_id': list(missing_sample_ids),
            'timepoint': timepoints
        }, index=list(missing_sample_ids))

        missing_adata = anndata.AnnData(X=zeros_data, var=adata.var, obs=missing_obs)
        combined_adata = adata.concatenate(missing_adata, batch_key=None, join='outer')
    else:
...
# /home/qiuaodon/Desktop/PanCancer_scRNA_analysis/utils/scRNA_utils.py

from scipy.stats import pearsonr, spearmanr
import statsmodels.api as sm
import pandas as pd

def calculate_gene_correlation(results, adata_1_pseudo, adata_2_pseudo, method='pearson'):
    ...
    for index, row in results.iterrows():
        ...
        if g1 not in adata_1_pseudo.var_names or g2 not in adata_2_pseudo.var_names:
            continue
...
def calculate_residuals_correlation(results, adata_T_pseudo, adata_M_pseudo, method='pearson'):
